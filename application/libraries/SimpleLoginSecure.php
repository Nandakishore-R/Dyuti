...
class SimpleLoginSecure
{
	...
	function login($user_email = '', $user_pass = '') 
	{
		...
		if ($query->num_rows() > 0) 
		{
			...
			$this->CI->db->simple_query('UPDATE ' . $this->user_table  . ' SET updated_at = now() WHERE id = ' . $user_data['id']);
			//Set session data
			unset($user_data['password']);
			$user_data['user'] 			= 	$user_data['email']; 		// for compatibility with Simplelogin
			$user_data['type'] 			= 	$user_data['user_type']; 
	...
	function webuserlogin($user_email = '', $user_pass = '') 
	{
		$this->CI =& get_instance();
